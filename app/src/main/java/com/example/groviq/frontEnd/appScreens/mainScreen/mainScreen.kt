...
fun mainScreen(mainViewModel : PlayerViewModel, playlistNavigationLocal: NavHostController)
{
    ...
    val playlistsAutoGenerated = mainUiState.audioData.entries
        .filter { it.value.autoGenerated }

    // запускаем LaunchedEffect один раз при первом композировании
    LaunchedEffect(mainUiState) {
        // DAILY_PLAYLIST_AUTOGENERATED
        val dailyPlaylist = playlistsAutoGenerated.find { it.key == "DAILY_PLAYLIST_AUTOGENERATED" }
        if (dailyPlaylist == null || dailyPlaylist.value.shouldBeRegenerated()) {
            AppViewModels.search.createDailyPlaylist(mainViewModel)
        }

        // MOOD_PLAYLIST_AUTOGENERATED_*
        val moodPlaylists = playlistsAutoGenerated.filter { it.key.startsWith("MOOD_PLAYLIST_AUTOGENERATED") }
        if (moodPlaylists.isEmpty() || moodPlaylists.any { it.value.shouldBeRegenerated() }) {
